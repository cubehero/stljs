 ...
 */
STL.file2PNG = function(filePath, options, callback) {
  if (options == null) options = {};
  if (options.width == null) options.width = 800;
  if (options.height == null) options.height = 800;

  var renderer = path.join(__dirname, "render_stl/render_stl.sh")
  var pathname = path.dirname(filePath);
  var basename = path.basename(filePath);

  var cmd = child_proc.spawn(renderer,
      [basename, "/tmp", options.width, options.height], { 
        cwd: pathname 
      });
  ...
  cmd.on('exit', function(code) {
    ...
    if (code == 0) {
      ...
      callback({
        errno: code, 
        description: "returning standard linux exit code",
        url: "http://stackoverflow.com/questions/1101957/are-there-any-standard-exit-status-codes-in-linux"
      })
    }
  });
}
